...
export class ClaudeFormatter {
  ...
  formatAgentList(agents, typeFilter = null) {
    if (agents.length === 0) {
      return typeFilter ? `No agents found for type: ${typeFilter}` : 'No agents available'
    }
  ...
  formatSearchResult(result) {
    if (!result.found) {
      ...
      if (result.suggestions && result.suggestions.length > 0) {
        ...
        result.suggestions.forEach((suggestion, index) => {
          const confidence = Math.round((suggestion.similarity || suggestion.confidence || 0) * 100)
          const description = suggestion.description || suggestion.type || 'No description'
          output += `${index + 1}. ${suggestion.name || 'Unknown'} - ${description} (${confidence}%)\n`
        })
    ...
    // Handle other result structures (e.g., from findWorkflow)
    if (result.result) {
      return this.formatFindResult(result.result, result.match_type || 'search')
    }
  ...
  /**
   * Format find result from semantic search
   */
  formatFindResult(result, matchType) {
    if (!result) {
    ...
    if (result.similarity !== undefined) {
      output += `**Confidence:** ${Math.round(result.similarity * 100)}%\n`
    }
    ...
    output += `\n💡 Load this context: lev get-workflow ${result.slug || result.name}`
