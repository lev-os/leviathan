...
class KinglyAgentServer {
  ...
  async initializeCommandRegistry() {
    try {
      ...
      console.error('Failed to initialize command registry:', error.message);
    }
  }

  setupToolHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      // Get auto-discovered tools from command registry
      ...
      return {
        ...
        // Auto-discovered tools from command registry
        ...autoDiscoveredTools
      ],
      };
    });
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        switch (request.params.name) {
          ...
          default:
            // Try auto-discovered commands from command registry
