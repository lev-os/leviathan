/**
 * Semantic Search Engine for Context Discovery
 * 
 * EXTRACTED FROM: mcp-mvp/src/semantic-lookup.js
 * UNIFIED FOR: Kingly Core Agent
 * 
 * Uses OpenAI embeddings for intelligent context discovery
 * Enhanced with rich YAML content extraction and type filtering
 * Returns slug-based results for clean context identification
 */
...
export class SemanticLookup {
  ...
  }  // Enhanced embedding text extraction with rich YAML content
  getEmbeddingText(context) {
    const parts = [
      ...
      // Rich YAML content extraction for better semantic search
      ...
      // MCP tools information for tool-specific contexts
      context.data.tool_config?.mcp_tools?.map(tool => 
        `${tool.name} ${tool.description}`
      ).join(' ') || '',
      
      // Integration notes provide implementation context
      context.data.tool_config?.integration_notes ? 
        Object.values(context.data.tool_config.integration_notes).join(' ') : '',
      
      // Response formatting info helps with tool usage patterns
