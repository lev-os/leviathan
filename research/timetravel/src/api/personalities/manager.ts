...
export class PersonalityManager {
  ...
  private async loadCorePersonalities(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to load core personalities:', error as Error)
    }
  }
  ...
  async analyzeWithPersonality(personalityId: string, findings: Finding[]): Promise<PersonalityAnalysis> {
  ...
  private async generateSynthesis(findings: AnalyzedFinding[], personality: PersonalityConfig): Promise<string> {
    const prompt = personality.prompts.synthesis.replace('{findings}', this.formatFindingsForPrompt(findings))
  ...
  async save(personality: PersonalityConfig): Promise<void> {
    const isCustom = personality.custom !== false
    const dir = isCustom ? 'custom' : 'core'
    const filePath = path.join(this.personalitiesPath, dir, `${personality.id}.yaml`)

    try {
      await fs.ensureDir(path.dirname(filePath))
      await fs.writeFile(filePath, yaml.stringify(personality), 'utf8')
      this.personalities.set(personality.id, personality)
      this.logger.info(`Saved personality: ${personality.name}`)
    } catch (error) {
      this.logger.error('Failed to save personality:', error as Error)
      throw error
    }
  }
}
