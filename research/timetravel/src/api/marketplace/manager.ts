...
export class MarketplaceManager {
  ...
  private async loadCommunityPersonalities(): Promise<void> {
    const communityPath = path.join(this.marketplacePath, 'community')

    try {
      const files = await fs.readdir(communityPath)

      for (const file of files) {
        if (file.endsWith('.yaml') || file.endsWith('.yml')) {
          const filePath = path.join(communityPath, file)
          const content = await fs.readFile(filePath, 'utf8')
          const personality = yaml.parse(content) as MarketplacePersonality

          personality.featured = false
          this.personalities.set(personality.marketplaceId, personality)
          this.logger.info(`Loaded community personality: ${personality.name}`)
        }
      }
    } catch (error) {
      this.logger.warn('No community personalities found')
    }
  }

  private async loadStrategies(): Promise<void> {
    // TODO: Implement strategy loading
    this.logger.info('Loading strategies (not yet implemented)')
  }

  private async loadBundles(): Promise<void> {
    // TODO: Implement bundle loading
    this.logger.info('Loading bundles (not yet implemented)')
  }
}
