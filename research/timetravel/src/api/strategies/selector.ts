...
export class PersonalitySelector {
  ...
  async selectOptimalPersonalities(
    ...
    for (const rule of sortedRules) {
      if (this.evaluateCondition(rule.condition, context)) {
        for (const personalityId of rule.personalities) {
          if (availablePersonalities.find(p => p.id === personalityId) && 
              !selectedPersonalities.includes(personalityId)) {
    ...
    const optimizedSelection = this.optimizeCombinations(
      selectedPersonalities,
      strategyConfig.combinations
    );
    
    // Ensure we have at least 3-4 personalities for comprehensive analysis
    const finalSelection = this.ensureMinimumCoverage(
      optimizedSelection,
  ...
  private determineOptimalStrategy(config: ResearchConfig): string {
    ...
    if (topic.includes('innovation') || topic.includes('breakthrough') || 
        topic.includes('technology') || topic.includes('research')) {
      return 'innovation';
    }
    
    if (topic.includes('finance') || topic.includes('investment') || 
        topic.includes('market') || topic.includes('business')) {
      return 'financial';
    }
  ...
  private buildContext(config: ResearchConfig): Record<string, any> {
    ...
    switch (type) {
      case 'topic_contains':
        return value.split('|').some(keyword => 
          context.topic.includes(keyword.toLowerCase())
        );
  ...
  private optimizeCombinations(
    ...
    for (const combo of combinations) {
      const hasAllPersonalities = combo.personalities.every(p => 
