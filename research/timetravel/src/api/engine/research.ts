...
export class ResearchEngine {
  ...
  async execute(config: ResearchConfig): Promise<ResearchResult> {
    ...
    try {
      ...
      result.synthesis = await this.synthesizeWithPersonalities(
        result.findings, 
    ...
    const streamResults = await Promise.all(
      streams.map(stream => this.executeStream(config, stream, 1))
    );
  ...
  private async executeTier2(config: ResearchConfig, tier1Findings: Finding[]): Promise<Finding[]> {
    ...
    const deepDiveResults = await Promise.all(
      deepDiveStreams.map(stream => this.executeStream(config, stream, 2))
    );
  ...
  private async executeTier3(config: ResearchConfig, allFindings: Finding[]): Promise<Finding[]> {
    ...
    for (const tool of stream.tools) {
      try {
        ...
        findings.push(...toolResults.map(result => ({
          id: uuidv4(),
          source: tool,
  ...
  private async synthesizeWithPersonalities(
    findings: Finding[], 
