...
function testPackageStructure() {
  console.log('🧪 Testing @lev/validation core package structure...\n');
  ...
  const tests = [
    {
      ...
      test: () => {
        ...
        // Check for @lev/debug dependency
        if (!packageData.dependencies['@lev/debug']) {
          throw new Error('Missing @lev/debug dependency');
        }
    ...
    {
      name: 'Source files contain @lev/debug imports',
      test: () => {
        const srcPath = path.join(__dirname, 'src');
        ...
        for (const file of sourceFiles) {
          ...
          if (!content.includes("from '@lev/debug'")) {
            throw new Error(`File ${file} missing @lev/debug import`);
          }
          ...
          if (!hasLogger && !hasTracer && !hasMonitor) {
            throw new Error(`File ${file} imports @lev/debug but doesn't use it`);
          }
        }
  ...
  if (failed === 0) {
    ...
    console.log('🔧 @lev/debug integration is properly implemented');
