...
export class UniversalTestPatterns {
    ...
    const validationTests = [
      {
        ...
        test: () => {
          return plugin.config?.plugin?.name && plugin.config?.plugin?.version && plugin.config?.plugin?.type
        },
      },
      {
        name: 'Has defined capabilities',
        test: () => {
          return Array.isArray(plugin.config?.capabilities) && plugin.config.capabilities.length > 0
        },
      },
      {
        ...
        test: () => {
          ...
          return (
            commands && Object.keys(commands).length > 0 && Object.values(commands).every((cmd) => cmd.syntax && cmd.description)
          )
        },
      },
      {
        ...
        test: () => {
          const commands = plugin.config?.commands
          return commands && Object.values(commands).some((cmd) => cmd.whisper?.strategies || cmd.whisper?.llm_guidance)
        },
      },
    ]
  ...
  findCommandsForCapability(plugin, capability) {
    ...
    return commandNames.filter((cmdName) => {
      ...
      return (
        cmdName.includes(capability.replace(/_/g, '')) ||
