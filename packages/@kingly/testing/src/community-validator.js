...
export class CommunityValidator {
  ...
  async scanGithubPlugins(githubToken) {
    ...
    try {
      ...
      for (const repo of data.items.slice(0, 10)) { // Limit to 10 repos
      ...
      for (const filePath of possiblePaths) {
        try {
          ...
          if (response.ok) {
            ...
            const config = this.parseYamlContent(content);
    ...
    const tests = [
      {
        ...
        test: () => {
          const commands = plugin.config?.commands || {};
          return Object.values(commands).every(cmd => 
            cmd.syntax && cmd.syntax.startsWith('kingly ')
          );
        }
      },
      {
        ...
        test: () => {
          ...
          const standardCapabilities = [
            'debugging', 'validation', 'process_management', 
            'memory_management', 'workflow_execution'
          ];
          return capabilities.some(cap => 
            standardCapabilities.some(std => cap.includes(std.replace('_', '')))
          );
        }
      },
      {
  ...
  async testConventionCompliance(plugin) {
    const tests = [
      ...
      },
      {
        ...
        test: () => {
          return plugin.config?.plugin?.description && 
                 plugin.config.plugin.description.length > 10;
        }
      }
    ];
