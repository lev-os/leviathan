...
export class PluginDiscovery {
  ...
  async discoverCorePlugins() {
    ...
    try {
      ...
      // Also check for @kingly scoped packages
      const kinglyPackages = await glob('@kingly/*/config/plugin.yaml', {
        cwd: this.corePackagesPath,
        absolute: true
      });
      
      for (const yamlFile of kinglyPackages) {
        try {
          const plugin = await this.loadPluginFromYaml(yamlFile);
          if (plugin) {
            plugin.type = 'core_plugin';
          ...
          logger.warn(`Failed to load @kingly plugin from ${yamlFile}`, { error: error.message });
        }
      }
  ...
  async buildDependencyGraph(plugins) {
    ...
    for (const plugin of plugins) {
      ...
      // Check for @kingly/* imports in package.json or source files
      if (plugin.type === 'core_plugin') {
        try {
          ...
          const dependencies = Object.keys(packageJson.dependencies || {})
            .filter(dep => dep.startsWith('@kingly/'))
            .map(dep => dep.replace('@kingly/', ''));
